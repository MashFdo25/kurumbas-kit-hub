...
const App = () => {
        ...
        <div className="flex flex-col items-center sticky top-10 space-y-12">
          <div id="jersey-preview" className="relative w-[380px] h-[520px] md:w-[440px] md:h-[600px] bg-slate-900 rounded-t-[100px] shadow-2xl overflow-hidden border-b-[60px] border-black">
            ...
            <div className="absolute top-[28%] w-full flex flex-col items-center px-12 text-center">
              <h2 className="text-white font-black text-4xl md:text-5xl uppercase mb-4">{formData.jerseyName || "JOHN"}</h2>
  ...
  if (view === 'success') return (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 text-center text-white font-sans overflow-hidden">
      ...
      <div className="flex flex-wrap gap-4 mt-8 justify-center"><button onClick={() => handleDownload('share-card', `KCC_Signing.png`)} className="bg-white text-black px-12 py-4 rounded-[2rem] font-black text-lg uppercase flex items-center gap-3 hover:bg-orange-500">Share My Signing</button><button onClick={() => setView('landing')} className="bg-slate-900 text-slate-500 px-8 py-4 rounded-[2rem] font-black text-lg uppercase hover:text-white">Home</button></div>
    </div>
  );
  ...
  if (view === 'admin-auth') return (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 font-sans">
      <div className="bg-slate-900 p-16 md:p-24 rounded-[5rem] border border-slate-800 shadow-2xl max-w-xl w-full text-center space-y-14">
        ...
        <button onClick={() => adminPass === 'TEAM2026' ? setView('admin') : alert('Denied')} className="w-full bg-orange-500 text-black py-8 rounded-[3rem] font-black uppercase text-2xl">Authenticate</button>
      </div>
    </div>
  );
  ...
  if (view === 'admin') return (
    <div className="min-h-screen bg-slate-950 p-6 md:p-12 text-white font-sans overflow-y-auto">
      <div className="max-w-7xl mx-auto space-y-12">
        <div className="flex justify-between items-end"><h1 className="text-6xl font-black text-orange-500 uppercase italic">Control</h1><div className="flex gap-4"><button onClick={() => setView('landing')} className="bg-slate-900 p-4 rounded-xl border border-slate-800 hover:text-white shadow-xl"><X size={24}/></button></div></div>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="bg-slate-900 p-8 rounded-[2.5rem] border border-slate-800 flex flex-col justify-between group hover:border-orange-500/50"><div className="flex justify-between items-start mb-4"><p className="text-[10px] text-slate-500 uppercase font-black">Jersey Price</p><Settings2 size={16} className="text-slate-700" /></div><div className="relative"><span className="absolute left-0 top-1/2 -translate-y-1/2 text-orange-500 text-3xl font-black italic">$</span><input type="number" value={extraJerseyPrice} onChange={(e) => setExtraJerseyPrice(e.target.value)} className="bg-transparent pl-8 border-none text-5xl font-black text-white w-full outline-none italic" /></div></div>
          <div className="bg-slate-900 p-8 rounded-[2.5rem] border border-slate-800"><p className="text-[10px] text-slate-500 uppercase font-black mb-4">Squad</p><p className="text-6xl font-black italic">{orders.length}</p></div>
