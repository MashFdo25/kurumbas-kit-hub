...
// --- NETLIFY / VITE ENVIRONMENT ADAPTER ---
const getEnv = (key) => {
  try {
...
// Initialize Firebase safely for Netlify build cycles
...
const App = () => {
  ...
  useEffect(() => {
    const initAuth = async () => {
      try {
        ...
        setErrorMessage("Connection Error: Check Netlify variables and Firebase Auth.");
        setDbStatus('error'); 
      }
    };
    initAuth();
  ...
  const generateAIPersona = async () => {
    ...
    setIsAiLoading(true);
    const prompt = `Suggest 3 nicknames for player "${formData.playerName}". Return format: NAME1, NAME2, NAME3`;
    const res = await callGemini(prompt, "Creative assistant.");
  ...
  const generateAdminBrief = async () => {
    ...
    const prompt = `Analyze ${orders.length} orders for Kurumbas CC.`;
  ...
  const exportManufacturerReport = async () => {
    ...
    const tableRows = orders.map(o => {
      // Force Uppercase for PDF
      const playerNameUpper = (o.playerName || "").toUpperCase();
      const jerseyNameUpper = (o.jerseyName || "").toUpperCase();

      const printDetails = `Name: ${jerseyNameUpper}\nNo: ${o.number}`;
      ...
      const formatKitItem = (kit) => {
        let details = [];
        if (kit.name) details.push(kit.name.toUpperCase());
      ...
      return [playerNameUpper, printDetails, squadSizes, family, extra];
    });
  ...
  const exportToExcel = async () => {
    ...
    orders.forEach(o => {
      // Force Uppercase for Excel
      const pUpper = (o.playerName || "").toUpperCase();
      const jUpper = (o.jerseyName || "").toUpperCase();

      rows.push(["Squad Bundle", pUpper, "Match Polo", jUpper, o.number, o.jerseySize, o.customJerseySize || ""]);
      rows.push(["Squad Bundle", pUpper, "Training Vest", jUpper, o.number, o.jerseySize, o.customJerseySize || ""]);
      rows.push(["Squad Bundle", pUpper, "Long Pants", "", "", o.pantSize, o.customPantSize || ""]);
      rows.push(["Squad Bundle", pUpper, "Training Shorts", "", "", o.shortSize, o.customShortSize || ""]);
      rows.push(["Squad Bundle", pUpper, "Training Skinny", "", "", o.skinnySize, o.customSkinnySize || ""]);

      o.familyKits?.forEach(fk => {
        rows.push(["Family Support", pUpper, "Match Polo", (fk.name || "").toUpperCase(), fk.number || "", fk.size, fk.customSize || ""]);
      });

      o.extraPaidJerseys?.forEach(ek => {
        rows.push(["Extra Paid Gear", pUpper, "Match Polo", (ek.name || "").toUpperCase(), ek.number || "", ek.size, ek.customSize || ""]);
      });
    });
  ...
  const handleOrderSubmit = async (e) => {
    ...
    try {
      ...
      const finalOrder = { ...formData, currentPriceAtOrder: extraJerseyPrice || 2800, timestamp: Date.now() };
      
      const writePromise = setDoc(doc(db, 'artifacts', projectAppId, 'public', 'data', 'orders', orderId), finalOrder);
      const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error("Database write timed out. Check Rules/Internet.")), 10000));
  ...
  if (view === 'landing') return (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 text-center overflow-hidden relative font-sans">
      ...
      <div className="z-10 flex flex-col items-center max-w-4xl">
        ...
        <h1 className="text-7xl md:text-[10rem] font-black text-white italic tracking-tighter uppercase mb-8 leading-none select-none">Kurumbas <span className="text-orange-500">CC.</span></h1>
        ...
        <button onClick={() => setView('customize')} className="group bg-white text-black px-12 md:px-20 py-7 rounded-[2.5rem] font-black text-2xl hover:bg-orange-500 transition-all flex items-center gap-4 shadow-2xl hover:-translate-y-1">ENTER HUB <ArrowRight className="group-hover:translate-x-2 transition-transform"/></button>
      ...
      <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 items-start">
        <div className="bg-slate-900 p-8 md:p-14 rounded-[3.5rem] border border-slate-800 shadow-2xl space-y-16">
          <form onSubmit={handleOrderSubmit} className="space-y-16">
                ...
                {(formData.familyKits || []).map(k => (
                  <div key={k.id} className="bg-slate-950 p-8 rounded-[2.5rem] border border-slate-800 flex flex-col gap-6 relative">
                    ...
                    <div className="grid grid-cols-3 gap-6">
                      <input placeholder="NAME" className="w-full bg-slate-900 border border-slate-800 p-4 rounded-xl text-xs uppercase font-black" value={k.name || ""} onChange={e => setFormData({...formData, familyKits: formData.familyKits.map(x => x.id === k.id ? {...x, name: e.target.value} : x)})} />
                      <input placeholder="NO" type="number" className="w-full bg-slate-900 border border-slate-800 p-4 rounded-xl text-xs font-black" value={k.number || ""} onChange={e => setFormData({...formData, familyKits: formData.familyKits.map(x => x.id === k.id ? {...x, number: e.target.value} : x)})} />
                      <select className="w-full bg-slate-900 border border-slate-800 p-4 rounded-xl text-xs font-black" value={k.size || "W-M"} onChange={e => setFormData({...formData, familyKits: formData.familyKits.map(x => x.id === k.id ? {...x, size: e.target.value} : x)})}>
              ...
              <div className="space-y-6">
                {(formData.extraPaidJerseys || []).map(k => (
                  <div key={k.id} className="bg-slate-950 p-8 rounded-[2.5rem] border border-slate-800 flex flex-col gap-6 relative">
                    ...
                    <div className="grid grid-cols-3 gap-6">
                      <input placeholder="NAME" className="w-full bg-slate-900 border border-slate-800 p-4 rounded-xl text-xs uppercase font-black" value={k.name || ""} onChange={e => setFormData({...formData, extraPaidJerseys: formData.extraPaidJerseys.map(x => x.id === k.id ? {...x, name: e.target.value} : x)})} />
                      <input placeholder="NO" type="number" className="w-full bg-slate-900 border border-slate-800 p-4 rounded-xl text-xs font-black" value={k.number || ""} onChange={e => setFormData({...formData, extraPaidJerseys: formData.extraPaidJerseys.map(x => x.id === k.id ? {...x, number: e.target.value} : x)})} />
            ...
            <button disabled={isSubmitting || !formData.playerName || !user} type="submit" className="w-full bg-white text-black py-8 rounded-[3rem] font-black text-3xl hover:bg-orange-500 transition-all shadow-2xl disabled:opacity-30 uppercase italic flex items-center justify-center gap-4">
        ...
        <div className="flex flex-col items-center sticky top-10 space-y-12">
          <div id="jersey-preview" className="relative group perspective-1000">
             <div className="relative w-[380px] h-[520px] md:w-[440px] md:h-[600px] bg-slate-900 rounded-t-[100px] shadow-[0_80px_160px_rgba(0,0,0,0.9)] overflow-hidden border-b-[60px] border-black transition-all">
                ...
                <div className="absolute top-[28%] w-full flex flex-col items-center px-12 text-center">
                   <h2 className="text-white font-black text-4xl md:text-5xl uppercase drop-shadow-[0_5px_15px_rgba(0,0,0,1)] break-all leading-tight mb-4">{formData.jerseyName || "JOHN"}</h2>
                ...
                <div className="absolute bottom-12 left-12 flex items-center gap-4"><img src={TEAM_LOGO} className="w-16 h-16 opacity-90" /><div className="h-12 w-px bg-white/20"></div><div><p className="text-white text-xs font-black uppercase tracking-widest">KCC Squad</p></div></div>
             </div>
          </div>
        </div>
      </div>
  ...
  if (view === 'success') return (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 text-center text-white font-sans">
      ...
      <h1 className="text-8xl md:text-9xl font-black italic uppercase mb-8 leading-none">LOCKED <span className="text-orange-500">IN.</span></h1>
      <div id="share-card" className="w-[360px] h-[640px] bg-black rounded-[4rem] overflow-hidden relative shadow-2xl border border-white/5 mb-10">
        ...
        <div className="absolute inset-0 p-16 flex flex-col justify-end text-left">
          ...
          <h2 className="text-8xl font-black italic uppercase leading-none mb-4">{lastOrder?.jerseyName || "JOHN"}</h2>
          <span className="text-orange-500 font-black text-7xl italic">#{lastOrder?.number || "00"}</span>
        </div>
      </div>
      <div className="flex gap-6"><button onClick={() => handleDownload('share-card', `KCC_Signing.png`)} className="bg-white text-black px-16 py-6 rounded-[3rem] font-black text-lg uppercase flex items-center gap-4 hover:bg-orange-500">Share My Signing</button><button onClick={() => setView('landing')} className="bg-slate-900 text-slate-500 px-12 py-6 rounded-[3rem] font-black text-lg uppercase hover:text-white transition-all">Home</button></div>
    </div>
  );
  ...
  if (view === 'admin-auth') return (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 font-sans">
      <div className="bg-slate-900 p-16 md:p-24 rounded-[5rem] border border-slate-800 shadow-2xl max-w-xl w-full text-center space-y-14">
        ...
        <input type="password" placeholder="••••" className="w-full bg-slate-950 border border-slate-800 p-8 rounded-[3rem] text-center text-7xl text-white outline-none focus:border-orange-500 transition-all font-mono"
  ...
  if (view === 'admin') return (
    <div className="min-h-screen bg-slate-950 p-8 md:p-14 overflow-y-auto text-white font-sans">
      <div className="max-w-7xl mx-auto space-y-20">
        <div className="flex justify-between items-end"><h1 className="text-6xl font-black text-orange-500 uppercase italic">Control</h1><div className="flex gap-6"><button onClick={() => setView('landing')} className="bg-slate-800 p-5 rounded-[2rem] hover:text-white transition-all shadow-xl"><X size={32}/></button></div></div>
        ...
        <div className="bg-slate-900 rounded-[5rem] border border-slate-800 overflow-hidden shadow-2xl">
          <div className="p-12 border-b border-slate-800 bg-slate-800/20 flex flex-col md:flex-row gap-6 justify-between items-center">
            ...
            <div className="flex gap-4">
              <button onClick={exportManufacturerReport} className="bg-white text-black px-8 py-4 rounded-2xl text-xs font-black uppercase flex items-center gap-3 hover:bg-orange-500 shadow-xl">PDF</button>
              <button onClick={exportToExcel} className="bg-green-600 text-white px-8 py-4 rounded-2xl text-xs font-black uppercase flex items-center gap-3 hover:bg-green-500 shadow-xl">Excel</button>
            </div>
          </div>
          <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-800">
                {orders.map(o => (
                  <tr key={o.id} className="text-white group hover:bg-slate-800/20 transition-all">
                    ...
                    <td className="p-12"><div className="flex flex-wrap gap-3"><span className="bg-slate-950 px-4 py-2 rounded-2xl text-xs font-black border border-slate-800">J:{o.customJerseySize || o.jerseySize}</span></div></td>
                    <td className="p-12 text-center">Fam: {o.familyKits?.length || 0}</td>
                    <td className="p-12 text-right"><button onClick={() => handleDeleteOrder(o.id)} className="bg-slate-800 p-5 rounded-[2rem] text-red-500 hover:bg-red-500 hover:text-white transition-all"><Trash2 size={28}/></button></td>
                  </tr>
                ))}
              </tbody>
            </table>
