...
import { 
  ...
  CheckCircle2, AlertTriangle, Database, Loader2, Settings2
} from 'lucide-react';
...
const App = () => {
          ...
          <form onSubmit={handleOrderSubmit} className="space-y-16">
            ...
            <div className="space-y-10">
              <div className="flex items-center justify-between border-b border-slate-800 pb-5">
                <div className="flex items-center gap-4"><div className="bg-pink-600 p-3 rounded-2xl"><Heart size={24} className="text-white"/></div><h3 className="text-3xl font-black uppercase tracking-tighter italic text-pink-500">3. Family Jersey</h3></div>
                <button type="button" onClick={() => setFormData({...formData, familyKits: [...(formData.familyKits || []), { id: Date.now(), name: '', number: '', size: 'W-M', customSize: '' }]})} className="bg-pink-600/10 border border-pink-600/20 text-pink-500 px-6 py-3 rounded-2xl text-xs font-black uppercase flex items-center gap-2 hover:bg-pink-600 hover:text-white shadow-xl transition-all"><PlusCircle size={18}/> Add Member</button>
              </div>
                ...
                {(formData.familyKits || []).map(k => (
                  <div key={k.id} className="bg-slate-950 p-8 rounded-[2.5rem] border border-slate-800 flex flex-col gap-6 relative animate-in slide-in-from-right">
                    ...
                    <div className="grid grid-cols-3 gap-6">
                      <input placeholder="NAME" className="w-full bg-slate-900 border border-slate-800 p-4 rounded-xl text-xs uppercase font-black outline-none focus:border-pink-600 transition-all" value={k.name || ""} onChange={e => setFormData({...formData, familyKits: formData.familyKits.map(x => x.id === k.id ? {...x, name: e.target.value} : x)})} />
                      <input placeholder="NO" type="number" className="w-full bg-slate-900 border border-slate-800 p-4 rounded-xl text-xs font-black outline-none focus:border-pink-600 transition-all" value={k.number || ""} onChange={e => setFormData({...formData, familyKits: formData.familyKits.map(x => x.id === k.id ? {...x, number: e.target.value} : x)})} />
              ...
              <div className="space-y-6">
                {(formData.extraPaidJerseys || []).map(k => (
                  <div key={k.id} className="bg-slate-950 p-8 rounded-[2.5rem] border border-slate-800 flex flex-col gap-6 relative animate-in slide-in-from-right">
                    ...
                    <div className="grid grid-cols-3 gap-6">
                      <input placeholder="NAME" className="w-full bg-slate-900 border border-slate-800 p-4 rounded-xl text-xs uppercase font-black outline-none focus:border-yellow-500 transition-all" value={k.name || ""} onChange={e => setFormData({...formData, extraPaidJerseys: formData.extraPaidJerseys.map(x => x.id === k.id ? {...x, name: e.target.value} : x)})} />
                      <input placeholder="NO" type="number" className="w-full bg-slate-900 border border-slate-800 p-4 rounded-xl text-xs font-black outline-none focus:border-yellow-500 transition-all" value={k.number || ""} onChange={e => setFormData({...formData, extraPaidJerseys: formData.extraPaidJerseys.map(x => x.id === k.id ? {...x, number: e.target.value} : x)})} />
            ...
            <button disabled={isSubmitting || !formData.playerName || !user} type="submit" className="w-full bg-white text-black py-8 rounded-[3rem] font-black text-3xl hover:bg-orange-500 transition-all shadow-2xl disabled:opacity-30 uppercase italic tracking-tighter flex items-center justify-center gap-4">
  ...
  if (view === 'success') return (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 text-center text-white font-sans overflow-hidden">
      ...
      <div className="flex flex-wrap gap-4 mt-8 justify-center"><button onClick={() => handleDownload('share-card', `KCC_Signing.png`)} className="bg-white text-black px-12 py-4 rounded-[2rem] font-black text-lg uppercase flex items-center gap-3 hover:bg-orange-500 transition-all shadow-2xl active:scale-95">Share My Signing</button><button onClick={() => setView('landing')} className="bg-slate-900 text-slate-500 px-8 py-4 rounded-[2rem] font-black text-lg uppercase hover:text-white transition-all shadow-xl">Home</button></div>
    </div>
  );
  ...
  if (view === 'admin-auth') return (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 font-sans">
      <div className="bg-slate-900 p-16 md:p-24 rounded-[5rem] border border-slate-800 shadow-2xl max-w-xl w-full text-center space-y-14">
        ...
        <input type="password" placeholder="••••" className="w-full bg-slate-950 border border-slate-800 p-8 rounded-[3rem] text-center text-7xl text-white outline-none focus:border-orange-500 transition-all font-mono" onChange={(e) => setAdminPass(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && adminPass === 'TEAM2026' && setView('admin')} />
  ...
  if (view === 'admin') return (
    <div className="min-h-screen bg-slate-950 p-6 md:p-12 text-white font-sans overflow-y-auto">
      <div className="max-w-7xl mx-auto space-y-12">
        <div className="flex justify-between items-end"><h1 className="text-6xl font-black text-orange-500 uppercase italic tracking-tighter leading-none">Control</h1><div className="flex gap-4"><button onClick={() => setView('landing')} className="bg-slate-900 p-4 rounded-xl border border-slate-800 hover:text-white shadow-xl transition-all"><X size={24}/></button></div></div>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="bg-slate-900 p-8 rounded-[2.5rem] border border-slate-800 flex flex-col justify-between group hover:border-orange-500/50 transition-all"><div className="flex justify-between items-start mb-4"><p className="text-[10px] text-slate-500 uppercase font-black tracking-widest">Jersey Price</p><Settings2 size={16} className="text-slate-700 transition-colors" /></div><div className="relative"><span className="absolute left-0 top-1/2 -translate-y-1/2 text-orange-500 text-3xl font-black italic">$</span><input type="number" value={extraJerseyPrice} onChange={(e) => setExtraJerseyPrice(e.target.value)} className="bg-transparent pl-8 border-none text-5xl font-black text-white w-full outline-none italic tracking-tighter" /></div></div>
          <div className="bg-slate-900 p-8 rounded-[2.5rem] border border-slate-800 shadow-2xl"><p className="text-[10px] text-slate-500 uppercase font-black mb-4 tracking-widest">Squad</p><p className="text-6xl font-black italic tracking-tighter">{orders.length}</p></div>
          <div className="bg-slate-900 p-8 rounded-[2.5rem] border border-slate-800 shadow-2xl"><p className="text-[10px] text-slate-500 uppercase font-black mb-4 tracking-widest">Prints Req</p><p className="text-6xl font-black italic tracking-tighter">{totalPrintsRequired}</p></div>
        </div>
        <div className="bg-slate-900 rounded-[3rem] border border-slate-800 overflow-hidden shadow-2xl">
          <div className="p-10 border-b border-slate-800 bg-slate-800/20 flex flex-col md:flex-row gap-6 justify-between items-center"><h4 className="text-xl font-black uppercase flex items-center gap-4"><FileText size={24} className="text-orange-500" /> Sponsoring Manifest</h4><div className="flex flex-wrap gap-4 items-center">
            <div className="relative group w-full md:w-64"><Search size={14} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 transition-colors" /><input type="text" placeholder="Search players..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-slate-950 border border-slate-800 pl-10 pr-4 py-2 rounded-xl text-xs outline-none focus:border-orange-500 transition-all" /></div>
            ...
            <button onClick={exportToExcel} className="bg-green-600 text-white px-6 py-2 rounded-xl text-[10px] font-black uppercase hover:bg-green-500 transition-colors shadow-lg">Excel</button>
          </div></div>
                ...
                {filteredOrders.map(o => (
                  <tr key={o.id} className="text-white group hover:bg-slate-800/10 transition-all">
                    ...
                    <td className="p-10"><div className="flex gap-2">
                        {o.familyKits?.length > 0 && <span className="bg-pink-500/10 text-pink-500 px-3 py-1.5 rounded-lg text-[9px] font-black border border-pink-500/20 uppercase tracking-widest">FAM: {o.familyKits.length}</span>}
                        {o.extraPaidJerseys?.length > 0 && <span className="bg-yellow-500/10 text-yellow-500 px-3 py-1.5 rounded-lg text-[9px] font-black border border-yellow-500/20 uppercase tracking-widest">PAID: {o.extraPaidJerseys.length}</span>}
                    </div></td>
                    <td className="p-10 text-right space-x-2"><button onClick={() => handleEditOrder(o)} className="bg-slate-800 p-4 rounded-xl text-slate-500 hover:text-orange-500 hover:bg-orange-500/10 transition-all"><Edit3 size={20}/></button><button onClick={() => handleDeleteOrder(o.id)} className="bg-slate-800 p-4 rounded-xl text-slate-500 hover:text-red-500 hover:bg-red-500/10 transition-all shadow-2xl active:scale-90"><Trash2 size={20}/></button></td>
                  </tr>
                ))}
              </tbody>
            </table>
